...
def runMakeCsv(ID,PdbFile,CAP,PdbDirectory,GeoA,GeoB,GeoC,GeoD,GeoE,GeoF,GeoAx,GeoBx,GeoCx,GeoDx,GeoEx,GeoFx,GeoAMin,GeoAMax,GeoBMin,GeoBMax,GeoCMin,GeoCMax,Inclusions,Exclusions):
    print('LeucipPipeline:02SixContacts - 1) Creating data -----------------------------------------------------------')
    ...
    data['ID_AB'] = data.apply(lambda row: help.applyID(row['aa'],row['rid'],row['chain'],row['atom'],row['aaA'],row['ridA'],row['chainA'],row['atomA'],row['aaB'],row['ridB'],row['chainB'],row['atomB']), axis=1)
    data['ID_AC'] = data.apply(lambda row: help.applyID(row['aa'],row['rid'],row['chain'],row['atom'], row['aaA'], row['ridA'],row['chainA'], row['atomA'], row['aaC'], row['ridC'], row['chainC'], row['atomC']),axis=1)
    data['ID_AD'] = data.apply(lambda row: help.applyID(row['aa'], row['rid'], row['chain'], row['atom'], row['aaA'], row['ridA'],row['chainA'], row['atomA'], row['aaD'], row['ridD'], row['chainD'], row['atomD']),axis=1)
    data['ID_AE'] = data.apply(lambda row: help.applyID(row['aa'], row['rid'], row['chain'], row['atom'], row['aaA'], row['ridA'],row['chainA'], row['atomA'], row['aaE'], row['ridE'], row['chainE'], row['atomE']),axis=1)
    data['ID_AF'] = data.apply(lambda row: help.applyID(row['aa'], row['rid'], row['chain'], row['atom'], row['aaA'], row['ridA'],row['chainA'], row['atomA'], row['aaF'], row['ridF'], row['chainF'], row['atomF']),axis=1)
    ...
    if GeoCMin != -1:
        data = data.query('`' + GeoC + '` >= ' + str(GeoCMin))
    if GeoCMax != -1:
        data = data.query('`' + GeoC + '` <= ' + str(GeoCMax))
    ...
    data['Coords0'] = data.apply(lambda row: help.applyCOORDS(geo, atomsdata, row['pdb_code'], row['info' + GeoA], 0),axis=1)
    data['CoordsA'] = data.apply(lambda row: help.applyCOORDS(geo, atomsdata, row['pdb_code'], row['info' + GeoA], 1),axis=1)
    data['CoordsB'] = data.apply(lambda row: help.applyCOORDS(geo, atomsdata, row['pdb_code'], row['info' + GeoB], 1),axis=1)
    data['CoordsC'] = data.apply(lambda row: help.applyCOORDS(geo, atomsdata, row['pdb_code'], row['info' + GeoC], 1),axis=1)
    data['CoordsD'] = data.apply(lambda row: help.applyCOORDS(geo, atomsdata, row['pdb_code'], row['info' + GeoD], 1),axis=1)
    data['CoordsE'] = data.apply(lambda row: help.applyCOORDS(geo, atomsdata, row['pdb_code'], row['info' + GeoE], 1),axis=1)
    data['CoordsF'] = data.apply(lambda row: help.applyCOORDS(geo, atomsdata, row['pdb_code'], row['info' + GeoF], 1),axis=1)

    data['AngleA0B'] = data.apply(lambda row: help.applyANGLE(row['CoordsA'], row['Coords0'],row['CoordsB']),axis=1)
    data['AngleA0C'] = data.apply(lambda row: help.applyANGLE(row['CoordsA'], row['Coords0'], row['CoordsC']), axis=1)
    data['AngleA0D'] = data.apply(lambda row: help.applyANGLE(row['CoordsA'], row['Coords0'], row['CoordsD']), axis=1)
    data['AngleA0E'] = data.apply(lambda row: help.applyANGLE(row['CoordsA'], row['Coords0'], row['CoordsE']), axis=1)
    data['AngleA0F'] = data.apply(lambda row: help.applyANGLE(row['CoordsA'], row['Coords0'], row['CoordsF']), axis=1)

    data = data.sort_values(by='AngleA0B', ascending=False)

    data.to_csv("Csv/" + ID + "_03_Geometry.csv", index=False)
    atomsdata.to_csv("Csv/" + ID + "_03_Atoms.csv", index=False)
    print('LeucipPipeline:02SixContacts - 2) Saved data to', "Csv/" + ID + "_03_Geometry.csv")
