...
aa_GLY = {'aa':['GLY']}
aa_PRO = {'aa':['PRO']}
aa_NO_gly_pro = ['ALA','CYS','ASP','GLU','PHE','HIS','ILE','LYS','LEU','MET','ASN','GLN','ARG','SER','THR','VAL','TRP','TYR']
...
def getGeos(onlyGLY,synthetic):
    ...
    if onlyGLY:
        if synthetic:
            return runMakeGeosPairwise(PWAtomsGLY) + OtherGLY
        else:
            return runMakeGeosPairwise(PWAtomsGLY) + OtherGLY + OtherDssp + OtherChain
    else:
        if synthetic:
            return runMakeGeosPairwise(PWAtomsGLY + PWAtoms) + OtherGLY + Other
        else:
            return runMakeGeosPairwise(PWAtomsGLY + PWAtoms) + OtherGLY + Other + OtherChain + OtherDssp
...
def trimGeos(data,trim, geoA,geoB):
    temp_data = data[[geoA,geoB]]
    for geo in [geoA,geoB]:
